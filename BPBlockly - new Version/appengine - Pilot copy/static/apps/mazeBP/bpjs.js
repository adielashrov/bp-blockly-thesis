...
/* Original compare bids operating on the b-thread's priority.
...
BProgram.prototype.run = function() {
    ...
    while (this.running.notEmpty()) {
		...
		var bt = bid.bthread;
        try {
            var newbid = bt.send(this.lastEvent); // Run an iteration of the generator
    ...
    if (this.lastEvent) {
        ...
        while (this.pending.notEmpty()) {
            ...
            for (var i=0; i<waitlist.length; i++) {
				...
				//Added code Adiel - support for multiple events 15.8.2013 - Do we need this?
				/*
...
BProgram.prototype.selectNextEvent = function() {
            ...
            if (bid.block) {
                for (k=0; k<bid.block.length; k++) {
                    ...
                    if (e == blocked ||
						...
						// Leave ok = true if e is in bid.request or in bid.wait
						if( bid.request.indexOf(e) == -1) {
							var waiting_for_e=false;
							for (l=0; l<bid.wait.length; l++) {
								var waiting = bid.wait[l];
								if (waiting == e ||
									(typeof(waiting) == 'function' && waiting(this.lastEvent))
								) {
									waiting_for_e = true;
    ...
    if (events.length > 0) {
        events.sort(compareBids);//Currently using compareBids based on static ordering in bpblocks.js 
